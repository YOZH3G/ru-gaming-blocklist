...
TARGET_REGIONS = {"eu-central-1", "eu-west-1"}
TARGET_SERVICE = "EC2"
STATIC_DOMAINS = [
    "api.bethesda.net",
    "bethesda.net",
...
OUTPUT = ROOT / "games" / "Fallout76_AWS.txt"
...
def fetch_aws_ranges() -> dict:
    request = urllib.request.Request(
        AWS_IP_RANGES_URL,
        headers={"User-Agent": "ru-gaming-blocklist-fallout76-updater/1.0"},
    )
...
def build_list(payload: dict) -> tuple[list[str], str | None]:
    ...
    if not networks:
        raise RuntimeError("AWS feed returned no matching Fallout 76 EC2 prefixes")
